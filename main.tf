...
module "scheduler_service_account" {
  ...
  name       = "${local.instance_alias_name}-scheduler"
}
...
module "workflow_service_account" {
  source     = "github.com/dapperlabs-platform/terraform-google-iam-service-account?ref=v1.1.8"
  project_id = var.project_name
  name       = "${local.instance_alias_name}-workflow"
}
...
module "workflow" {
  source                 = "github.com/dapperlabs-platform/terraform-google-cloud-workflows?ref=mgardner-bump-service-module"
  project_id             = var.project_name
  ...
  workflow_trigger = {
    cloud_scheduler = {
      name                  = "${local.instance_alias_name}-backup-job"
